  ...
  void ServiceControl::CreateAndStart(CreateArgs args)
  {
    ...
    if (service == NULL)
    {
      ...
      if (service == NULL)
      {
        ...
        EmitState("denied");
        std::cout << "wireguard_2govpn: Failed to create the service" << GetLastError() << std::endl;
    ...
    if (!ChangeServiceConfig2(service, SERVICE_CONFIG_SERVICE_SID_INFO, &sid_type))
    {
      ...
      CloseServiceHandle(service_manager);
      EmitState("denied");
      std::cout << "wireguard_2govpn: Failed to configure servivce SID type" << GetLastError() << std::endl;
    ...
    if (!ChangeServiceConfig2(service, SERVICE_CONFIG_DESCRIPTION, &description))
    {
      ...
      CloseServiceHandle(service_manager);
      EmitState("denied");
      std::cout << "wireguard_2govpn: Failed to configure service description" << GetLastError() << std::endl;
            ...
            &dwBytesNeeded))                // size needed if buffer is too small
    {
      ...
      CloseServiceHandle(service_manager);
      EmitState("denied");
      std::cout << "wireguard_2govpn: Failed to query service status" << GetLastError() << std::endl;
      return;
    }
    ...
    if (ssStatus.dwCurrentState != SERVICE_STOPPED && ssStatus.dwCurrentState != SERVICE_STOP_PENDING)
    {
      ...
      CloseServiceHandle(service_manager);
      EmitState("connected");
      std::cout << "wireguard_2govpn: Service is already running" << GetLastError() << std::endl;
      return;
    }
    ...
    {
      std::cout << "wireguard_2govpn: Failed to start the service: " << GetLastError() << std::endl;

      if (args.first_time)
      {
        std::cout << "wireguard_2govpn: Trying to delete and recreate the service" << std::endl;
        EmitState("reconnect");
