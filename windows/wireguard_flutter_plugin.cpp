...
namespace wireguard_2govpn
{
  ...
  void WireguardFlutterPlugin::RegisterWithRegistrar(PluginRegistrarWindows *registrar)
  {
    auto channel = make_unique<MethodChannel<EncodableValue>>(
        registrar->messenger(), "young.yang.wireguard_2govpn/wgcontrol", &StandardMethodCodec::GetInstance());
    auto eventChannel = make_unique<EventChannel<EncodableValue>>(
        registrar->messenger(), "young.yang.wireguard_2govpn/wgstage", &StandardMethodCodec::GetInstance());
